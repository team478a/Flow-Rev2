...
| 0021 | 認証メールテンプレート | テーブル＋一意インデックス＋RLS | ✅ | ✅ |

0021 の適用確認（2026-08-03）: テーブル `true` / 一意インデックス `2` / ポリシー `2`。新規テーブルの作成のみで既存データには触れないため、バックアップ無しで適用した。**このテーブルが空でもコード内の既定テンプレートで認証メールは送信される。**
