...
### 5.3 パスワードリセット — ✅ 実施済み（prod・2026-08-03）

| 確認項目 | 結果 |
|---|---|
| `/reset-password` からメール送信 | ✅ 届いた |
| リンク → `/auth/confirm` → `/update-password` | ✅ 着地 |
| 新パスワードの設定 | ✅ |
| `/login` から新パスワードでログイン | ✅ |

**この導線は、修正前は一度も完了できなかった。** 原因は3つ重なっていた。

| # | 原因 | 節 | 修正 |
|---|---|---|---|
| 1 | PKCEトークンは `verifyOtp` で検証できない | §4-0 | PR #24 |
| 2 | テンプレートの `type` が `invite` のまま | §4-0-B | PR #25（コード側が `type` を決める形にして構造的に解消） |
| 3 | リンク先 `/auth/update-password` が存在しない | §4-1 | PR #23 |

3つとも「送信もエラーにならず、画面上も正常に見える」種類の失敗で、**リンクを踏んだ利用者だけが詰まる**。1つ直しても次が出るため、切り分けにVercelのログ出力（PR #24）が必要だった。
