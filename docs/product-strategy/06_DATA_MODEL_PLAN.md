...
| `ai_provider_settings` / `email_settings` / `line_accounts` / `stripe_accounts` / `cloudflare_settings` | reuse-existing | テーブル構造は概ね維持しつつ、読み取りロジック（「テナント別設定があれば使い、なければ上位へフォールバック」）を統一する。**（2026-08-01決定事項、2026-08-02訂正）到達できる階層数はテーブルのテナント列構成に依存する: `ai_provider_settings`/`email_settings`は`white_label_id`のみで`client_id`列がなく2階層（WL→HQ）が上限。`line_accounts`/`stripe_accounts`は`client_id`・`white_label_id`の両列が既存のため3階層（クライアント→OEM→本部）を実現できる。`cloudflare_settings`はテナント列が一切ないため、Phase 1で追加方法を確定したうえで3階層化する（`08`文書Task 1-4、`09`文書参照）** |
